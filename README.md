...
実際にテストした結果に基づく推奨モデルリストです。

### 🥇 プレミアム（VRAM 22-24GB）

| モデル | VRAM | 特徴 |
|--------|------|------|
| **Qwen3-30B-A3B** | 22GB | 安定・高品質・深い思考。最も推奨 |
...
| モデル | VRAM | 特徴 |
|--------|------|------|
| **Qwen3 14B** | 18GB | バランス型。内蔵Reasoningあり |

- 「熟考せよ」「思索せよ」等のキーワードでMCP Sequential Thinkingを使用
- 通常の質問では内蔵Reasoningを使用（MCPを使わない場合あり）
- 日本語品質は良好
...
| モデル | VRAM | 特徴 |
|--------|------|------|
| **gpt-oss (21B-A3.6B)** | 11GB | 軽量・高速（160 tokens/sec） |

- MCPツールの呼び出しは可能
- ⚠️ **注意**: 検索結果の解釈や記憶の活用が浅い傾向あり
- お試し用・動作確認用として推奨
- プロンプトの工夫で改善できる可能性あり
...
| Gemma 3 12B | MCPツールを呼び出さない。23GB VRAMで非効率。速度も遅い |
